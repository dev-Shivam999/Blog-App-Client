...
const Setimg = memo(({ fo, im, cl ='w-full' }: {cl?:string, fo: string, im: (e: React.ChangeEvent<HTMLInputElement>) => void }) => {
    ...
    return (
        <div className={`flex gap-3 ${cl}  my-3 mx-auto items-center `}>
            <div className='w-1/2 mx-auto h-[30vh] bg-gray-300 rounded-lg relative'>
                <img src={fo || ""} className='w-full h-full rounded-full object-fill object-top absolute top-0 left-0' alt="" />
